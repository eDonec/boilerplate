...
const packagesFolder = workspaces
...
const appsWithTestScript = allPackagesWithTestScripts.filter(package => package.jsonPath.includes('apps'))
  .map(package => ({
    packageName: package.name,
    packagePath: `apps/${package.jsonPath.split('/apps/')[1].replace('/package.json', '/**')}`,
    isApp: true
  }))
const packagesWithTestScripts = allPackagesWithTestScripts.filter(package => !package.jsonPath.includes('apps'))
  .map(package => ({
    packageName: package.name,
    packagePath: `packages/${package.jsonPath.split('/packages/')[1].replace('/package.json', '/**')}`,
    isApp: false
  }))
